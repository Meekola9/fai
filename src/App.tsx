...
function Header() {
  ...
  const nav = viewerMode ? NAV.filter((item) => item.to !== '/entry' && item.to !== '/data') : NAV
  return (
    <header className="sticky top-0 z-40 border-b border-line bg-ink/90 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-3 py-3 sm:px-4">
        <NavLink to="/" aria-label="FAI dashboard"><Brand /></NavLink>
        <nav className="hidden items-center gap-1 md:flex">
          {nav.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => `rounded-lg px-2.5 py-1.5 text-xs font-semibold transition ${isActive ? 'bg-fai/15 text-fai' : 'text-muted hover:bg-panel-2 hover:text-chalk'}`}
            >
              {item.label}
            </NavLink>
          ))}
          ...
          {viewerMode && (
            <NavLink to="/login" className="ml-1 rounded-lg border border-line px-3 py-1.5 text-xs font-bold text-muted hover:bg-panel-2 hover:text-chalk">Coach Sign In</NavLink>
          )}
        </nav>
        <div className="flex items-center gap-2 md:hidden">
          <ConnectivityBadge />
          <NavLink to="/vertical" className="grid h-9 min-w-9 place-items-center rounded-lg border border-fai/30 bg-fai/5 px-2 text-[10px] font-black text-fai" aria-label="Open vertical standards">VERT</NavLink>
          <NavLink to="/stats" className="grid h-9 min-w-9 place-items-center rounded-lg border border-line bg-panel px-2 text-[10px] font-black text-muted" aria-label="Open FAI guides">GUIDES</NavLink>
          <NavLink to="/tv" className="grid h-9 min-w-9 place-items-center rounded-lg border border-flame/40 bg-flame/10 px-2 text-xs font-black text-flame" aria-label="Open TV Mode">TV</NavLink>
        </div>
      </div>
    </header>
  )
    ...
    <div className="grid min-h-screen place-items-center bg-ink px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border border-line bg-panel p-6 shadow-2xl">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-black text-chalk">Coach Sign In</h1><p className="text-sm text-muted">Secure team cloud storage</p></div>
        </div>
        {(formError || authError) && <div className="mb-4 rounded-xl border border-down/40 bg-down/5 p-3 text-sm text-down">{formError || authError}</div>}
        <form onSubmit={submit} className="space-y-4">
          ...
          <label className="block"><span className="mb-1 block text-xs font-bold uppercase tracking-wider text-muted">Password</span><input type="password" autoComplete="current-password" required value={password} onChange={(event) => setPassword(event.target.value)} className="w-full rounded-xl border border-line bg-ink px-4 py-3 text-chalk outline-none focus:border-fai" /></label>
          <button type="submit" disabled={busy} className="w-full rounded-xl bg-fai px-5 py-3 font-black text-ink disabled:cursor-wait disabled:opacity-60">{busy ? 'Signing in…' : 'Sign in to FAI'}</button>
        </form>
        <p className="mt-4 text-xs leading-relaxed text-muted">Use the Supabase account assigned to your team. Athlete records are protected by team membership and row-level security.</p>
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-ink px-4">
      <div className="w-full max-w-lg rounded-2xl border border-down/40 bg-panel p-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted">{authError ?? 'This account is not linked to an FAI team.'}</p>
        {userEmail && <p className="mt-3 text-xs text-muted">Signed in as {userEmail}</p>}
        <button type="button" onClick={() => void signOut()} className="mt-5 rounded-xl border border-line px-4 py-2 text-sm font-bold text-chalk hover:bg-panel-2">Sign out</button>
      </div>
    </div>
  )
}
...
export default function App() {
  ...
  if (isTv) return <Routes><Route path="/tv" element={<TVMode />} /></Routes>

  const guarded = (element: React.ReactElement) => viewerMode ? <Navigate to="/login" replace /> : element
  ...
  return (
    <div className="min-h-screen pb-[calc(5rem+env(safe-area-inset-bottom))] md:pb-0">
      ...
      <footer className="mx-auto hidden max-w-7xl px-4 pb-10 pt-4 text-center text-xs text-muted md:block">
        FAI — Football Athlete Index · {viewerMode ? 'Live team view · read only' : storageMode === 'cloud' ? 'Secure cloud storage with on-device backup' : 'Local-first with on-device backup'}
      </footer>
      <PwaControls />
