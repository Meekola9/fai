# FAI Archetype Film Model Library

This library maps every current FAI archetype ID to professional and college film models. The comparisons are teaching references—not claims that testing alone proves technique, instincts, production, or football IQ.

## How to use the library

- **Professional model:** shows the mature version of the role.
- **College film model:** provides a second, often more developmentally accessible example.
- **Study focus:** tells the athlete what to watch instead of merely watching highlights.
- Use full-game or all-snaps film whenever possible. Confirm the FAI athletic profile with position-specific film before treating an archetype as final.
...
| Archetype ID | FAI name | Professional model | College film model | Study focus |
|---|---|---|---|---|
| `wr-field-stretcher` | Field Stretcher | Tyreek Hill | Jameson Williams — Alabama | Stack releases, speed control, tracking deep balls |
...
| Archetype ID | FAI name | Professional model | College film model | Study focus |
|---|---|---|---|---|
| `ol-anchor-tackle` | Anchor Tackle | Trent Williams | Penei Sewell — Oregon | Independent hands, anchor recovery, edge-rush range |
| `ol-road-grader` | Road Grader | Quenton Nelson | Tyler Smith — Tulsa | Displacement, double-team movement, finish without overextension |
...
| Archetype ID | FAI name | Professional model | College film model | Study focus |
|---|---|---|---|---|
| `dl-gap-plugger` | Gap Plugger | D.J. Reader | Jordan Davis — Georgia | Double-team anchor, gap integrity, block recognition |
| `dl-penetrator` | Penetrator | Aaron Donald | Ed Oliver — Houston | First-step win, hand quickness, backfield angle |
| `dl-bull-rusher` | Bull Rusher | Chris Jones | Derrick Brown — Auburn | Long-arm conversion, power through half-man, pocket collapse |
| `dl-bend-specialist` | Bend Specialist | Calais Campbell | Arik Armstead — Oregon | Length through edges, hip flexibility, reducing surface area |
| `dl-two-gapper` | Two-Gapper | Vita Vea | Dexter Lawrence — Clemson | Lockout, peek-and-shed, controlling both adjacent gaps |
| `dl-twitch-freak` | Twitch Freak | Ed Oliver | Jalen Carter — Georgia | Sudden lateral entry, counter quickness, gap disruption |
| `dl-motor-guy` | Motor Guy | Maxx Crosby | Braden Fiske — Florida State | Pursuit after initial loss, second effort, snap-to-snap strain |
...
| Archetype ID | FAI name | Professional model | College film model | Study focus |
|---|---|---|---|---|
| `s-center-field-eraser` | Center Field Eraser | Earl Thomas | Malik Hooker — Ohio State | Middle-of-field range, route overlap, high-point angles |
| `s-box-enforcer` | Box Enforcer | Derwin James Jr. | Jamal Adams — LSU | Alley fit, block defeat, controlled impact tackling |
| `s-nickel-chess-piece` | Nickel Chess Piece | Antoine Winfield Jr. | Jalen Pitre — Baylor | Slot matching, pressure disguise, run/pass role switching |

## Kicker / Punter Specialist Fallbacks

| Archetype ID | FAI name | Professional model | College film model | Study focus |
|---|---|---|---|---|
| `kp-explosive-leg` | Explosive-Leg Specialist | Justin Tucker | Harrison Butker — Georgia Tech | Approach consistency, leg speed, contact point |
| `kp-coverage-speed` | Speed-Coverage Specialist | Jake Elliott | Evan McPherson — Florida | Kickoff coverage transition, open-field leverage, safe tackling |
| `kp-durable-dual` | Durable Dual Specialist | Michael Dickson | Matt Araiza — San Diego State | Repeated-leg quality, kickoff/punt workload, technique under fatigue |
| `kp-mobile-placement` | Mobile Placement Athlete | Johnny Hekker | Tory Taylor — Iowa | Rollout punt mechanics, directional placement, movement consistency |
| `kp-balanced` | Balanced Specialist | Younghoe Koo | Rodrigo Blankenship — Georgia | Repeatable operation, placement consistency, general specialist athleticism |

## Film-room implementation requirements

Each future in-app film record should preserve this structure:

```ts
interface ArchetypeFilmModel {
  archetypeId: string
  playerName: string
  level: 'NFL' | 'College' | 'Historical'
  fitTier: 'primary' | 'trait' | 'body-type' | 'development'
  studyTopics: string[]
  sourceUrl?: string
  clipStartSec?: number
  clipEndSec?: number
...
1. Do not automatically label a player from film-model similarity alone.
2. Do not use highlights as the only evidence for processing, blocking, coverage, or consistency.
3. Keep the athlete's FAI archetype ID unchanged; film confirms or challenges the projection.
4. Permit multiple models for one archetype and multiple archetypes for one player.
5. Store exact timestamps and coaching notes so athletes reach the teaching point immediately.
6. Prefer legally embeddable or coach-owned film links; never imply FAI owns third-party footage.
